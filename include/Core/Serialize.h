...
const int kColorImageByteSize =
    kImageCols * kImageRows * kColorImageChannels * sizeof(char);
const int kDepthImageByteSize =
    kImageCols * kImageRows * kDepthImageChannels * sizeof(ushort);

const int kSkipLengthOfColorImage = sizeof(char[10]) +    // sensor type
                                    sizeof(int) +         // version
...
inline RawDataFrames Read(std::istream& in) {
  ...
  const std::string head =
      ReadString(in, static_cast<unsigned int>(kRawDataFrameHeader.size()));
