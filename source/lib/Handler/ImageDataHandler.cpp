...
namespace NiS {
    ...
    void ImageHandler2::ConvertToPointImages() {
        ...
        for (auto i = 0; i < raw_data_frames_.size(); ++i) {
            ...
            kf.SetPointImage(std::move(ConvertDepthImageToPointImage(raw_data_frames_[i].depth_image)));
    ...
    PointImage ImageHandler2::ConvertDepthImageToPointImage(DepthImage const &depth_image) {
        ...
        for (auto row = 0; row < depth_image.rows; ++row) {
            for (auto col = 0; col < depth_image.cols; ++col) {
                cv::Point3f p = ScreenToWorld(col, row, depth_image.at<ushort>(row, col));
    ...
    PointImage ImageHandler2::ConvertDepthImageToPointImage(DepthImage const &depth_image, int choice) {
        ...
        switch (choice) {
            ...
            case 0: {
                converter = new XtionCoordinateConverter;
                break;
            }
            case 1: {
//				converter = new AistCoordinateConverter ( "" );
                break;
            }
