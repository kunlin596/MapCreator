...
namespace NiS {
    ...
    WorldPoint XtionCoordinateConverter::ScreenToWorld(ScreenPoint const &screen_point, ushort const depth) {
    ...
    ScreenPoint XtionCoordinateConverter::WorldToScreen(WorldPoint const &world_point) {

        const auto x =
                (world_point.x / (world_point.z * universal_xz_factor_) + 0.5f) * XtionFrameProperty::kXtionWidth;
        const auto y =
                (world_point.y / (world_point.z * universal_yz_factor_) + 0.5f) * XtionFrameProperty::kXtionHeight;

        return ScreenPoint(x, y);
    }

    WorldPoint AistCoordinateConverter::ScreenToWorld(ScreenPoint const &screen_point, ushort const depth) {
        ...
        if (depth > 0) {
            ...
            const float gx =
                    xz_factor * (static_cast< float >( screen_point.x ) / XtionFrameProperty::kXtionWidth - 0.5f);
            const float gy =
                    yz_factor * (static_cast< float >( screen_point.y ) / XtionFrameProperty::kXtionHeight - 0.5f);
    ...
    ScreenPoint AistCoordinateConverter::WorldToScreen(WorldPoint const &world_point) {
