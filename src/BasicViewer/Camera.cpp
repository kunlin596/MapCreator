...
void Camera::SetView(float eye_x, float eye_y, float eye_z, float center_x,
                     float center_y, float center_z, float up_x, float up_y,
                     float up_z) {
...
glm::mat4 Camera::ViewMatrix() const { return view_mat_; }
...
void Camera::MouseUpdate(const glm::vec2& mouse_delta) {
  look_at_ = glm::mat3(glm::rotate(glm::mat4(), -mouse_delta.x * 0.1f, up_)) *
             look_at_;
  look_at_ = glm::mat3(glm::rotate(glm::mat4(), mouse_delta.y * 0.1f,
                                   glm::cross(up_, look_at_))) *
             look_at_;
  view_mat_ = glm::lookAt(camera_position_, camera_position_ + look_at_, up_);
}
...
void Camera::KeyUpdate(const Move& move, const float rate) {
  ...
  glm::vec3 head_direction =
      glm::normalize(glm::cross(look_at_, vertical_direction));
...
void Camera::Reset() {
  SetView(0.0f, 0.5f, 5.0f, 0.0f, 0.0f, -0.5f, 0.0f, 1.0f, 0.0f);
}
