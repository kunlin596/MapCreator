...
Inliers::Inliers(QOpenGLFunctions_4_1_Core *GL, const Points &points1,
...
void Inliers::Render() {
  ...
  GLint transformation_matrix_uniform_location = GL->glGetUniformLocation(
      shader_program_->programId(), "transformation_matrix");
...
void Inliers::SetupData() {
  ...
  GL->glBufferData(GL_ARRAY_BUFFER, sizeof(VertexGL) * data_.size(),
                   reinterpret_cast<float *>(data_.data()), GL_DYNAMIC_DRAW);
...
void Inliers::DataFormatConversionHelper(const Points &points1,
                                         const Points &points2) {
  data_.clear();
